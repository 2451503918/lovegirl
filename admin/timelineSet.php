...
if ($connect) {
    ...
    if ($stmt) {
        ...
        $resTimeline = mysqli_stmt_get_result($stmt);
        while ($row = mysqli_fetch_assoc($resTimeline)) {
            $timelineList[] = $row;
        }
        mysqli_stmt_close($stmt);
    }
}
$timelineCount = count($timelineList);
...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <h4 class="header-title mb-3 size_18">轨迹管理
                    <a href="/admin/timelineAdd.php">
                    ...
                    <button type="button" class="btn btn-secondary btn-sm btn-rounded margin_left">
                        共<b><?php echo $timelineCount ?></b>条
                    </button>
                </h4>
                <table id="basic-datatable" class="table dt-responsive nowrap" width="100%">
                    ...
                    foreach ($timelineList as $item):
                        $SerialNumber++;
                        $typeLabel = $typeLabels[$item['type'] ?? ''] ?? ($item['type'] ?? '未知');
                    ?>
                    <tr>
                        <td>
                            <div class="SerialNumber"><?php echo $SerialNumber ?></div>
                        </td>
                        <td><span class="badge badge-primary-lighten"><?php echo htmlspecialchars($typeLabel, ENT_QUOTES, 'UTF-8') ?></span></td>
                        <td><?php echo htmlspecialchars($item['title'] ?? '', ENT_QUOTES, 'UTF-8') ?></td>
                        <td>
                            <div class="textHide index" style="max-width:250px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="<?php echo htmlspecialchars($item['content'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                                <?php echo htmlspecialchars($item['content'] ?? '', ENT_QUOTES, 'UTF-8') ?>
                            </div>
                        </td>
                        <td><?php echo htmlspecialchars($item['date'] ?? '', ENT_QUOTES, 'UTF-8') ?></td>
                        <td><?php echo htmlspecialchars($item['location'] ?? '', ENT_QUOTES, 'UTF-8') ?: '<span class="text-muted">无</span>' ?></td>
                        <td>
                            <a href="modTimeline.php?id=<?php echo intval($item['id']) ?>">
                                <button type="button" class="btn btn-secondary btn-rounded">
                                    <i class="mdi mdi-clipboard-text-play-outline mr-1"></i>修改
                                </button>
                            </a>
                            <form method="POST" action="delTimeline.php" style="display:inline" onsubmit="return confirm('确认删除此轨迹吗？')">
